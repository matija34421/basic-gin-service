...
func (s *ClientService) GetClients() ([]*clientdto.ResponseClientDto, error) {
	clients, err := s.clientRepository.GetClients()
...
func (s *ClientService) GetClientById(id int) (*clientdto.ResponseClientDto, error) {
	if id < 0 {
		return nil, fmt.Errorf("id parameter cant be nil or negative")
	}

	client, err := s.clientRepository.GetClientById(id)
...
func (s *ClientService) CreateClient(saveDto *clientdto.SaveClientDto) (*clientdto.ResponseClientDto, error) {
	if saveDto == nil {
	...
	savedClient, err := s.clientRepository.CreateClient(&client)
...
func (s *ClientService) UpdateClient(updateDto *clientdto.UpdateClientDto) (*clientdto.ResponseClientDto, error) {
	if updateDto == nil {
		return nil, fmt.Errorf("client dto cant be nil")
	}
	...
	if _, err := s.clientRepository.GetClientById(clientForUpdate.ID); err != nil {
	...
	updatedClient, err := s.clientRepository.UpdateClient(&clientForUpdate)
...
func (s *ClientService) DeleteClient(id int) error {
	...
	if _, err := s.clientRepository.GetClientById(id); err != nil {
